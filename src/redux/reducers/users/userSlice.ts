...
export const login = createAsyncThunk('authSlice/admin-login', async (user: UserState, thunkAPI) => {
    try {
        ...
        // Return the modified user object along with the role
        return { user: { ...res.user, role }, res }; // Return both user and response
    } catch (error: any) {
        return thunkAPI.rejectWithValue(error?.response?.data)
    }
})
...
//         // console.log((res.user.roles[0].id));
//         if(res.user.roles[0].name==="admin"){
//             user.role="admin";
//         }else{
//             user.role="user";
...
//         console.log(res);
        ...
        //     navigate('/admin/products'); // Assuming you have a route for admin dashboard
        // } else {
...
const slice = createSlice({
    ...
    reducers: {
        ...
        saveAddress: (state, action: PayloadAction<any>) => {
            ...
            userService.saveUserAddress(action.payload);
        }
    },

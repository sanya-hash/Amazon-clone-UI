...
const ProductCard:
    ...
    //         else {
    //             setLoading(true);
    //         const isInWishlist = wishlist?.find((item: IProductState) => item.id === id);
    ...
    //             });
    //         } else {
    //             console.log("adding in wishlist...");
    //             dispatch(addToWishlist(id)).then(() => {
    //                 setLoading(false);
    //             });
    //         }
            ...
            <div className={`${isHidden === true ? "inline sm:hidden" : "null"}`}>
                <div className="sm:w-[270px]  sm:h-[400px]  h-[250px]  max-h-full relative group flex w-screen sm:block sm:rounded-lg border-b border-black sm:border-none  overflow-hidden md:cursor-pointer dark:bg-[#f5f5f5] bg-[#ffffff]  shadow-lg">
                    <div className="relative flex items-center justify-center w-[60%] sm:w-auto">
                        ...
                        <div onClick={() => ToWishlist(data?.id as string)} className="tooltip sm:absolute hidden sm:block sm:top-4 sm:right-4 cursor-pointer hover:bg-[#c4bfbf] rounded-full p-2 " >
                            {isLoading ?
                                <MoonLoader
                                    size={20}
                                    color="#361AE3"
                    ...
                    <div className="w-full h-full p-[15px] relative ">
                        ...
                        <div onClick={() => ToWishlist(data?.id as string)} className="absolute block  sm:hidden top-4 right-4 cursor-pointer" >
                            {isLoading ?
                                <MoonLoader
                                    size={20}
                                    color="#361AE3"
                        ...
                        <Link to={`/product/${data?.id}`}>
                            <span className="absolute top-[120px] text-[#1c1b1b] font-[1rem] font-Rubik">$ {data?.price}</span>
